...
# ── File patterns to exclude from post-exec tracking ────────────────────
# Only editor temp files, Python bytecode, and system hidden files are
# skipped — they are unambiguously transient artefacts.  Script files
# (.py, .sh, .bat, etc.) are NOT filtered because a user may legitimately
# ask the agent to generate them via shell commands.

_SKIP_TRACK_SUFFIXES: frozenset[str] = frozenset({
    ".pyc", ".pyo",      # Python bytecode
    ".swp", ".swx", ".swo",  # Vim swap
    ".tmp",              # generic temp
})

_SKIP_TRACK_NAMES: frozenset[str] = frozenset({
    ".DS_Store", "Thumbs.db",
})


def _is_trackable_output_file(sandbox_path: str) -> bool:
    """Return True if *sandbox_path* is a user-facing asset worth tracking.

    Filters only transient artefacts:
    - Hidden files (names starting with ``.``)
    - Editor swap files (``.swp``, ``.swx``, ``.swo``)
    - Python bytecode (``.pyc``, ``.pyo``)
    - Generic temp files (``.tmp``)
    """
    name = sandbox_path.rpartition("/")[2]
    if not name:
        return False
    if name in _SKIP_TRACK_NAMES:
        return False
    if name.startswith("."):
        return False
    # Also skip backup files ending with ~ (e.g. Emacs)
    if name.endswith("~"):
        return False
    lower = name.lower()
    for suffix in _SKIP_TRACK_SUFFIXES:
...
class ExecTool(Tool):
    ...
    async def _execute_in_sandbox(
        ...
        *,
        ...
        # Snapshot workspace files before command so we can discover newly
        # created assets (e.g. PDFs generated by a script) afterwards.
        before_files: set[str] = set()
        try:
            _, find_out, _ = await sandbox.run_command(
                "find /home/miqi/workspace -type f 2>/dev/null",
                timeout=10.0,
            )
            if find_out.strip():
                before_files = set(find_out.strip().splitlines())
        except Exception:
            before_files = set()  # best-effort — non-fatal
        ...
        # ── Track newly created workspace files ──────────────────────
        # When a shell command generates output files (e.g. a Python
        # script that produces a PDF), those files are invisible to the
        # Task Assets panel because the file-system tools (write_file /
        # edit_file) were never called.  Diff the workspace before/after
        # and persist any new files so they appear in the UI.
        if not cancelled and not timed_out and exit_code == 0 and before_files:
            try:
                _, after_out, _ = await sandbox.run_command(
                    "find /home/miqi/workspace -type f 2>/dev/null",
                    timeout=10.0,
                )
                after_files: set[str] = set(after_out.strip().splitlines()) if after_out.strip() else set()
                new_files = after_files - before_files
                if new_files:
                    workspace = sandbox._log_workspace
                    from miqi.session.manager import SessionManager
                    sm = SessionManager(workspace)
                    for fpath in sorted(new_files):
                        # Only track files actually under the workspace
                        if not fpath.startswith("/home/miqi/workspace/"):
                            continue
                        if not _is_trackable_output_file(fpath):
                            continue
                        rel = fpath[len("/home/miqi/workspace/"):]
                        if rel:
                            try:
                                sm.save_tracked_file(session_key or "", rel, op="write")
                                logger.info(
                                    "ExecTool: tracked new file session=%s path=%s",
                                    session_key, rel,
                                )
                            except Exception:
                                pass
            except Exception:
                pass  # best-effort — never fail the command for tracking
