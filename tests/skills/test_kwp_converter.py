...
class TestBodyConversion:
    ...
    def test_connectors_table_simplified(self):
        ...
        result = convert_body(body)
        assert "## MiQi Tools (Standalone Mode)" in result
    ...
    def test_miqi_footer_added(self):
        body = "# Test Skill\n\nDo something."
        result = convert_body(body)
        assert "## Using This Skill with MiQi" in result
        assert "MiQi includes built-in tools" in result
...
class TestOutputFormat:
    """Verify the MiQi skill format is compatible with SkillsLoader."""
    ...
    def test_full_roundtrip(self):
        ...
        # Connectors replaced
        assert "MiQi Tools (Standalone Mode)" in output
