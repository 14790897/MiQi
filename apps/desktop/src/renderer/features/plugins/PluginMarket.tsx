...
export function PluginMarket() {
  ...
  return (
    <div className="p-4 max-w-2xl">
      ...
      {plugins.length === 0 ? (
        <p className="text-gray-400">No plugins installed. Add plugins to ~/.miqi/plugins/ or &lt;workspace&gt;/.miqi/plugins/.</p>
      ) : (
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <span className={cn(
                    ...
                    p.status === 'error' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600',
                  )}>
                    {p.status}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => handleToggle(p.name, p.status)} className="text-[var(--text-muted)] hover:text-[var(--text)]">
                    {p.status === 'active' ? <ToggleRight size={18} className="text-green-500" /> : <ToggleLeft size={18} />}
                  </button>
                  <button onClick={() => handleUninstall(p.name)} className="text-red-400 hover:text-red-600">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
