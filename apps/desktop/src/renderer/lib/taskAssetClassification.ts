 ...
 *
 * WHITELIST RULES (user-specified 2026-08-13 — MOF journey):
 ...
 * 3. deliverable formats (excel/word/pdf only: xls/xlsx/doc/docx/pdf) → results,
 *    regardless of op (a READ of a deliverable usually means a subprocess wrote
 *    it and the agent inspected it — the write was invisible to tracking).
 * 4. everything else (md/json/csv/html/txt/py/…) → process.
 *
 * Results sorted lastSeen DESC (newest first); process ASC.
 */

/** Deliverable document whitelist — excel/word/pdf ONLY (user-specified). */
export const DELIVERABLE_EXT_RE = /\.(?:docx?|xlsx?|pdf)$/i;
