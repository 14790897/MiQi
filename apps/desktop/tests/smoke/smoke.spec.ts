...
test.describe('Search Functionality', () => {
  ...
  /**
   * Helper: navigate to a page by clicking the sidebar button.
   */
  ...
  test.describe('SkillsPage search', () => {
    ...
    test('search filters skills by name', async ({ page }) => {
      ...
      await expect(page.getByText('pdf-generator')).toBeVisible({ timeout: 3000 });
      await expect(page.getByText('data-analyzer')).toBeVisible({ timeout: 3000 });

      const searchInput = page.getByPlaceholder('搜索技能…');
      await searchInput.fill('pdf');

      await expect(page.getByText('pdf-generator')).toBeVisible({ timeout: 3000 });
      await expect(page.getByText('code-reviewer')).not.toBeVisible({ timeout: 3000 });
      await expect(page.getByText('data-analyzer')).not.toBeVisible({ timeout: 3000 });
    });

    test('search filters skills by description', async ({ page }) => {
      await injectMockAndGoto(page);
      await navigateTo(page, '技能');
  ...
  test.describe('ExperiencePage search', () => {

    test('rules search input is visible', async ({ page }) => {
      await injectMockAndGoto(page);
      await navigateTo(page, '经验');

      // Click the "规则" tab (default tab is "facts")
      await page.getByRole('button', { name: '规则' }).click();
      await page.waitForTimeout(300);

      const searchInput = page.getByPlaceholder('搜索规则...');
      await expect(searchInput).toBeVisible({ timeout: 5000 });
    });

    test('rules search filters by title', async ({ page }) => {
      await injectMockAndGoto(page);
      await navigateTo(page, '经验');
      await page.getByRole('button', { name: '规则' }).click();
      await page.waitForTimeout(300);

      await expect(page.getByText('Python type hints')).toBeVisible({ timeout: 5000 });
      await expect(page.getByText('Error handling pattern')).toBeVisible({ timeout: 3000 });

      const searchInput = page.getByPlaceholder('搜索规则...');
      await searchInput.fill('Python');

      await expect(page.getByText('Python type hints')).toBeVisible({ timeout: 3000 });
      await expect(page.getByText('Error handling pattern')).not.toBeVisible({ timeout: 3000 });
    });

    test('rules search filters by content', async ({ page }) => {
      await injectMockAndGoto(page);
      await navigateTo(page, '经验');
      await page.getByRole('button', { name: '规则' }).click();
      await page.waitForTimeout(300);
      ...
      const searchInput = page.getByPlaceholder('搜索规则...');
      await searchInput.fill('try-catch');

      await expect(page.getByText('Error handling pattern')).toBeVisible({ timeout: 3000 });
      await expect(page.getByText('Python type hints')).not.toBeVisible({ timeout: 3000 });
    });
    ...
    test('history search filters results', async ({ page }) => {
      await injectMockAndGoto(page);
      await navigateTo(page, '经验');
      ...
      await expect(page.getByText('Database migration')).toBeVisible({ timeout: 5000 });
      await expect(page.getByText('React best practices')).not.toBeVisible({ timeout: 3000 });
    });

  });

  test.describe('Missing search features', () => {

    test('SessionExplorer has no search bar (known gap)', async ({ page }) => {
      await injectMockAndGoto(page);

      // Sidebar session section should NOT contain any search input
      const searchInput = page.locator('input[placeholder*="搜索" i]');
      await expect(searchInput).toHaveCount(0, { timeout: 3000 });
    });
