/**
 * E2E: Subagent spawn → execute → result callback
 *
 * Verifies the complete subagent lifecycle end-to-end in a real Electron
 * session by exercising the bridge agent API directly (deterministic —
 * no LLM tool-call variance):
 *
 *   1. Create a session via chat.send so the runtime is initialized
 *   2. window.miqi.agents.spawn(...) creates a code-agent
 *   3. The subagent runs through AgentControl → AgentJobRuntime →
 *      _run_agent() → SubAgentCompletedEvent
 *   4. The bridge drain loop forwards the event as "subagent_result"
 *   5. ChatConsole renders the result with ✅ / ❌ status
 *   6. agent.list shows the spawned agent with correct status
 *
...
test.describe('Subagent Spawn E2E', () => {
  ...
  // completion callback (added for the chat:subagent_result link) emits
  // status="aborted", which ChatConsole renders as ❌.
  ...
  // ── Test 5: AI uses the spawn tool via chat (issue #246 core) ────
  //
