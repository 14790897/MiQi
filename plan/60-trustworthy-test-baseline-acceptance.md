...
| F5 | *(this commit)* | docs(plan): correct plan 60 acceptance after review |
...
- **`tests/execution/test_bwrap_real_integration.py`** — Real bubblewrap integration test gated by `require_bwrap`
...
### Modified test files
- **`tests/conftest.py`** — Repository-local per-process pytest base temp + global autouse `isolated_process_environment` fixture + capability detection fixtures
...
# 10 errors — PermissionError: C:\Users\lishi\AppData\Local\Temp\pytest-of-lishi
```
...
### Command 1: Path contract tests
```
.venv/Scripts/python.exe -m pytest tests/test_paths.py -q
```
**14 passed, 0 failed, 0 skipped**

### Command 2: Consumer behavior + source audit
```
.venv/Scripts/python.exe -m pytest tests/test_miqi_home_consumers.py -q
```
**9 passed, 0 failed, 0 skipped**

### Command 3: Capability marker tests
```
.venv/Scripts/python.exe -m pytest tests/test_capability_markers.py -q
```
**7 passed, 0 failed, 1 skipped** (`require_wsl` skips on native Windows without WSL)
...
```
**1751 passed, 6 skipped, 32 deselected, 1 warning**  
Warning is a pre-existing Windows asyncio unclosed-transport warning in `test_exec_tool_sandbox_selection.py::test_legacy_path_with_sandbox_manager`.

### Command 5: Subprocess tests only (subprocess, no sandbox)
```
.venv/Scripts/python.exe -m pytest -q -m "subprocess and not sandbox"
```
**31 passed, 0 failed, 0 skipped, 1758 deselected**

### Command 6: Bwrap sandbox tests
```
.venv/Scripts/python.exe -m pytest -q -m "sandbox and bwrap"
```
**1 skipped, 0 failed, 1788 deselected**  
Skipped because `bwrap` is not available on native Windows.
...
| 5 | pytest default runs do not read/write real MiQi home | **PASS** — repository-local base temp + global autouse isolation fixture; no real `~/.miqi` writes observed |
...
| 9 | Normal + subprocess tests all green in capable environment | **PASS** — 1751 + 31 passed, 0 failed on native Windows |
...
All changes are committed. Untracked/ignored entries (`!! .claude/`, `!! .pytest-basetemp*/`, etc.) are visible with `git status --ignored`; they are not staged and `.claude/` was not modified or committed.
