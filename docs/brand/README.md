...
| 辅助品牌色 | `#0B7F91` | 渐变/点缀、进行中标签（`--tag-inprogress-bg`） |

## 2. 功能色（两套主题共用同一语义色）

| 语义 | 色值 | CSS 变量 |
|---|---|---|
| 成功 | `#10B981` | `--success` / `--success-text` |
| 提示 | `#F59E0B` | `--warning` / `--bypass` |
| 错误 | `#FF6161` | `--danger` |
| 执行 | `#3B82F6` | `--info` |
| 失效 | `#999999` | 禁用态（现有实现用 `disabled:opacity-40/50` 表达，视觉等效） |
